...
NS_INLINE BOOL hasNativeFSR(void) {
	static BOOL hasNativeFSR = NO;
	static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wselector"
		hasNativeFSR = [NSURL instancesRespondToSelector:@selector(fileSystemRepresentation)];
...
{
    if (hasNativeFSR()) {
        return [self fileURLWithFileSystemRepresentation:path isDirectory:isDir relativeToURL:baseURL];
    }
...
{
    if (hasNativeFSR()) {
        return (self = [self initFileURLWithFileSystemRepresentation:path isDirectory:isDir relativeToURL:baseURL]);
    }
    
	if (!baseURL.isFileURL) {
		return (self = (__bridge_transfer NSURL *)CFURLCreateFromFileSystemRepresentation(NULL, (const UInt8 *)path, (CFIndex)strlen(path), !!isDir));
	}
    
	return (self = (__bridge_transfer NSURL *)CFURLCreateFromFileSystemRepresentationRelativeToBase(NULL, (const UInt8 *)path, (CFIndex)strlen(path), !!isDir, (__bridge CFURLRef)baseURL));
}
...
- (const char *)su_fileSystemRepresentation
{
	...
	void *(^createBuf)(NSUInteger) = ^(NSUInteger len){
        void *buf = NSZoneMalloc(NSDefaultMallocZone(), len);
		if (!buf) {
...
{
    if (hasNativeFSR()) {
        return [self getFileSystemRepresentation:buffer maxLength:maxBufferLength];
    }

	CFURLRef URL = (__bridge CFURLRef)self;
	return !!CFURLGetFileSystemRepresentation(URL, YES, (UInt8 *)buffer, (CFIndex)maxBufferLength);
}
