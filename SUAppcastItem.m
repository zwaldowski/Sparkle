...
- initWithDictionary:(NSDictionary *)dict failureReason:(NSString**)error
{
	...
	if (self)
	{
		...
		id(^bail)(NSString *) = ^id(NSString *inError){
		...
		if(!newVersion )
		{
			return (self = bail(@"Feed item lacks sparkle:version attribute, and version couldn't be deduced from file name (would have used last component of a file name like AppName_1.3.4.zip)"));
		}
		...
		{
			return (self = bail(@"No enclosure in feed item"));
		}

		NSString*	enclosureURLString = [enclosure objectForKey:@"url"];
		if( !enclosureURLString && !theInfoURL )
		{
			return (self = bail(@"Feed item's enclosure lacks URL"));
		}
